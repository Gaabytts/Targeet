...
window.addEventListener("DOMContentLoaded", function() {
    if (location.href.search("pt-br") > 0) {
        document.querySelectorAll(".select-language a")[0].classList.add("language-selected");
    } else {
        document.querySelectorAll(".select-language a")[1].classList.add("language-selected");
    }
});
...
//Botão de radio
var radio = document.querySelector(".aradio");

function mostrar() {
    var vetorsexo, mensagem;
    vetorsexo = document.getElementsByName("rsexo");

    if (vetorsexo[0].checked)
        mensagem = "*Sexo Masculino indicado";
    else if (vetorsexo[1].checked)
        mensagem = "*Sexo Feminino indicado";
    else
        mensagem = "*Nenhum sexo indicado";
...
var viewSenha = document.querySelector(".viewSenha");
...
envia.addEventListener('click', () => {
    ...
    if (!senha.value == confirmaSenha.value) {
        console.log('oi');
        viewSenha.textContent = '*Confirme a Senha';
        view2.classList.add('campo-cpf-muda');
    }
});